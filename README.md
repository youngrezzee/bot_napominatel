...
- выгружает все таблицы бота в чат для бекапа
- восстанавливает бота из тех же JSON-файлов, если отправить их обратно в чат
...
- после выбора людей умеет спрашивать, какие периоды напоминаний нужны
- автоматически удаляет старые напоминания по расписанию
- автоматически чистит старые записи из базы раз в полгода
...
- `/backup` - выгрузить все таблицы бота в чат отдельными JSON-файлами
...
Бот отправит в чат JSON-файлы со всеми таблицами:
...
- отправь эти же JSON-файлы обратно в чат
- бот сам распознает их
- после получения полного набора таблиц восстановит базу и заново пересоберет расписание напоминаний

Файлы можно отправлять по одному и в любом порядке.
...
После выбора людей бот отдельно предлагает периоды напоминаний:
...
Если ничего не выбрать на этом шаге, бот автоматически включает все периоды по умолчанию.
...
Политика очистки базы такая:

- прошедшие события и связанные записи о напоминаниях удаляются из базы раз в 180 дней
